...
(async () => {
  document.getElementById("loader").innerText = "Cargando modelo...";
  modelo = await tf.loadLayersModel("Modelo/model.json");
  document.getElementById("loader").innerText = "Modelo cargado";
})();
...
function procesarCamara() {
  ...
  ctx.translate(tamano, 0); // Mover el origen al extremo derecho del canvas
  ctx.scale(-1, 1); // Invertir horizontalmente
  ctx.drawImage(video, 0, 0, tamano, tamano, 0, 0, tamano, tamano);
  ctx.restore(); // Restaurar el estado previo del contexto
...
function cambiarCamara(camaraId) {
  if (currentStream) {
